...
| Field | Value |
|-------|-------|
| Commit sha (short) | `f99a6b3` (post-6.4a-impl targeted regen — closes B-066). **This regen scope:** §1 + §6 + §7 + §11 + §13 (the 5 sections with deltas since `7109542`); §2 / §4 / §5 / §8 / §9 / §10 / §12 carried forward verbatim per LD-5. **8-commit staleness gap from `7109542`** (`95bb3c5` drift cleanup + `d5f3d17` §6+ regen pass + `309f6c4` slice 6.4 spec author + `bdc70c7` SESSION-STATE backfill + `db676f4` SESSION-STATE backfill + `cbf878f` slice 6.4a in-slice spec amendment + `b0806d0` slice 6.4a impl + `317b8b6` slice 6.4a SHA backfill); below the LD-1 ~10-commit threshold so targeted, not full. **Deltas this pass:** §6 admin-route block rewritten (`/admin` parent now redirects to `/admin/cards`; nested children `/admin/cards`, `/admin/decks`, `/admin/lessons`, `/admin/analytics` mount inside `<AdminLayout>` `<Outlet />`; `/admin/audit` intentionally absent per spec #04 §12 D-14 — FE consumer never built); component graph 69 → 70 (+`admin/AdminLayout`); §7 `AdminPanel.tsx` row replaced with three new `pages/admin/*` rows (AdminCards, AdminDecks, AdminLessons); §11 drift item 18 flipped to ✅ RESOLVED by `95bb3c5`, +3 NEW items (#19 split-impl spec #04 status convention, #20 BE 520→510 mystery regression, #21 D-026 process lesson cross-ref); §13 phase-6 row 3 → 4 (+`04-admin-authoring.md` partial-shipped row), per-phase total 83 → 84 across 7 phases. **Lineage chain closing the 8-commit gap from `7109542`:** `17bf188` (§3 + §4 regen, 2026-04-26) → `7109542` (§1 + §2 + §5 regen, 2026-04-26) → `d5f3d17` (§6 + §7 + §8 + §9 + §10 + §11 + §12 + §13 regen, 2026-04-27, slice 6.3 final pass) → THIS commit (§1 + §6 + §7 + §11 + §13 targeted, 2026-04-27, post-slice-6.4a). |
| Branch | `main` (60 commits ahead of `origin/main`; not yet pushed) |
| Generated | 2026-04-27 (targeted regen, 5 of 13 sections — §1, §6, §7, §11, §13). LD-1 from B-049: targeted regen since gap = 8 commits, well below ~10 threshold. LD-2: counts via filesystem enumeration (`find` / `wc`, not estimation). LD-3: ambiguous fields flagged "unknown — flag for next regen" rather than guessed. LD-5: section content replaced (in scope) / carried forward verbatim (out of scope). |
...
| Frontend pages | 23 — **+3 since prior §1 anchor `7109542`**: slice 6.3 added `Lesson.tsx` (`ba00331`); slice 6.4a deleted `pages/AdminPanel.tsx` (extract path per spec #04 §12 D-12) and added `pages/admin/AdminCards.tsx` (extracted from AdminPanel monolith) + `pages/admin/AdminDecks.tsx` + `pages/admin/AdminLessons.tsx` (placeholders for slice 6.4b). Net: `20 + Lesson + (3 admin/* − AdminPanel) = 23`. New `pages/admin/` subdirectory created — slice 6.4b will populate `AdminDeckDetail`, `AdminLessonEditor`, `AdminQuizItems` here. Verified via `find hirelens-frontend/src/pages -name "*.tsx" -not -path "*/__tests__/*" \| wc -l = 23`. |
| Frontend components | 70 (excl. `__tests__/*`) — **+3 since prior §1 anchor `7109542`**: slice 6.3 added `lesson/LessonRenderer.tsx` + `lesson/QuizItemPanel.tsx` (`ba00331`); slice 6.4a added `admin/AdminLayout.tsx` (`b0806d0` — multi-route admin shell with sidebar + `<Outlet />`, wrapped by `<AdminGate>` in App.tsx). New `components/admin/` subdirectory created. `find hirelens-frontend/src/components -name "*.tsx" -not -path "*/__tests__/*" \| wc -l = 70`. |
...
| Specs | 84 across 7 phases (phase-0=6, phase-1=13, phase-2=8, phase-3=11, phase-4=6, phase-5=36, phase-6=4) — see §13. **+4 phase-6 specs since prior §1 anchor `7109542`**: #01 foundation-schema (`309f6c4`-era, slice 6.1), #02 fsrs-quiz-item-binding (slice 6.2), #03 lesson-ux (slice 6.3), #04 admin-authoring (slice 6.4 spec author + slice 6.4a in-slice amendment `cbf878f`). Phase-5 unchanged at 36 since prior regen. |
| Tests | BE **510** (was 489 at prior §1 anchor — net `+21` masking a `-10` mystery regression: slice 6.1 +5 (489 → 494), slice 6.2 +19 (494 → 513), slice 6.3 +7 (513 → 520), then `-10` between slice 6.3 close and slice 6.4 spec-author start in `tests/test_study_api.py::TestDailyStatusPreflight` + `tests/test_usage_limits.py` — surfaced this regen, see §11 drift item 20). FE **345** (was 328 — `+17`: `+10` B-063 lesson-ux Lesson.test.tsx + QuizItemPanel.test.tsx, `+7` B-064 slice 6.4a tests/admin/AdminLayout.test.tsx + AdminCards.test.tsx). FE test-file count 56 → 58 (+2 in `tests/admin/`). Counts per close-lines; not re-run this slice (R14 exception (a), doc-only). |
...
| lesson_service.py | **NEW — Phase 6 slice 6.3 (`ba00331`).** Thin wrapper over `app/data/lesson_fixtures.py` (D-2 — per-instance Pydantic at module import; 2 decks / 3 lessons / 6 quiz_items spanning all three `question_type` values). All four functions accept `db: AsyncSession` as a forward-compat affordance per D-4 (ignored in 6.3 — `del db`; slice 6.4 swaps each function body to a DB query without touching route handlers or response shapes). `get_lesson_with_quizzes` bundles `LessonResponse` + ordered active quiz_items + lifted `deck_id` / `deck_slug` / `deck_title`; returns `None` when the lesson does not exist or is archived OR the parent deck cannot be resolved. `get_deck_lessons_bundle` returns `None` for missing/archived deck; empty `lessons` list (200) when deck has zero lessons. The fixture file's module docstring records slice-6.4 retirement plan (deletion + DB-query swap in the same commit). | get_lesson_with_quizzes, get_deck_with_meta, list_lessons_in_deck, get_deck_lessons_bundle | — |
...
| `/admin/decks` | `<AdminDecks>` (lazy) — placeholder | AppShell + AdminLayout `<Outlet />` | ProtectedRoute → `AdminGate` (inherited) | Phase 6 slice 6.4a placeholder ("Coming in slice 6.4b"). Real `<AdminDecks>` editor lands in slice 6.4b (B-065). |
| `/admin/lessons` | `<AdminLessons>` (lazy) — placeholder | AppShell + AdminLayout `<Outlet />` | ProtectedRoute → `AdminGate` (inherited) | Phase 6 slice 6.4a placeholder ("Pick a deck to author lessons"). Real `<AdminLessonEditor>` (mounted at `/admin/lessons/:lesson_id`) lands in slice 6.4b (B-065). |
...
### Component graph (70 components, organized by directory)

| Directory | Components |
|-----------|-----------|
| **`admin/`** *(NEW — Phase 6 slice 6.4a, `b0806d0`)* | **AdminLayout** *(NEW — multi-route admin shell. Renders `<nav aria-label="Admin sections">` sidebar with 4 `<NavLink>` children (Cards / Decks / Lessons / Analytics; `/admin/audit` absent per spec #04 §12 D-14) + `<Outlet />` for nested admin route children. Wrapped externally by `<ProtectedRoute><AdminGate>` in `App.tsx` per AC-4. Active link surfaces via `aria-current="page"` (set automatically by react-router `<NavLink>`). Consumes `<PageWrapper>` for chrome. Slice 6.4b will populate this directory with `MarkdownEditor.tsx` + `ConfirmCascadeModal.tsx` per spec #04 §8.2.)* |
| `auth/` | AdminGate |
...
| **admin/AdminDecks.tsx** *(NEW — Phase 6 slice 6.4a, `b0806d0`)* | AdminDecks | — | — | — | Placeholder per AC-3 ("Deck authoring — Coming in slice 6.4b"). Real deck-list / deck-detail pages land in slice 6.4b (B-065) per spec #04 §8.2. |
| **admin/AdminLessons.tsx** *(NEW — Phase 6 slice 6.4a, `b0806d0`)* | AdminLessons | — | — | — | Placeholder per AC-3 ("Lesson authoring — Pick a deck to author lessons"). Real `/admin/lessons/:lesson_id` editor lands in slice 6.4b (B-065) per spec #04 §8.2. |
...
High-signal output — re-verified against HEAD `f99a6b3` this targeted regen (carry-forward of 18 prior items, 1 status flip ✅ RESOLVED on item 18, 3 NEW entries surfaced this pass — items 19, 20, 21). **Reconciled against post-B-048 SOP state:** R3 = "Never skip auth" (auth only); R19 = push-back; R18 retired (merged into R15(c)); SOP-8 = concurrent-session detection (codified `2504d6b`); SOP-9 = no concurrent CC sessions on one tree (added by B-048, `e2714b4`); H1–H4 = chat-Claude ↔ CC handoff section (added by B-048).
...
19. **NEW — Spec status convention gap for split-impl specs.** Spec #04 (`docs/specs/phase-6/04-admin-authoring.md`) was authored 2026-04-27 (`309f6c4`) covering both slice 6.4a (B-064) and slice 6.4b (B-065) per §12 D-1 split. Slice 6.4a shipped via `b0806d0` on 2026-04-27 (closes B-064); slice 6.4b is pending impl at B-065. Spec body Status line currently reads `Drafted, not shipped` — that's now ambiguous. Neither `Drafted, not shipped` nor `Shipped (spec + impl) — closes B-0##` (the canonical post-ship form #58 uses) accurately describes the partial-shipped state. **Recommendation (R-class candidate, not yet locked):** add a new canonical Status string `Partially shipped — slice Na shipped (closes B-### by <sha>); slice Nb pending B-###` to the §13 status legend, and flip spec #04's Status line to that form on its next legitimate edit (NOT in this regen — CR is read-only audit artifact, doesn't edit spec source). Future split-impl specs would adopt the same convention. **Status: NEW — surfaced this regen.** Tracked as recommendation only; no BACKLOG row pre-allocated.

20. **NEW — BE test count regression `520 → 510` between slice 6.3 close and slice 6.4 spec-author start.** 10 tests in `tests/test_study_api.py::TestDailyStatusPreflight` + `tests/test_usage_limits.py` flipped from passing to failing without any code touching their surfaces in the intervening commits (only `309f6c4` slice 6.4 spec-author + `bdc70c7` SESSION-STATE backfill landed between them; both doc-only). Slice 6.4a (FE-only impl per AC-6) did not change the count. Failing tests per slice 6.4a Step 0 baseline: `test_force_regenerate_bypasses_cache`, `test_new_generation_decrements_free_tier`, `test_usage_free_user_zero_history`, `test_usage_free_user_at_interview_prep_cap_shows_zero_remaining`, `test_free_user_under_cap_can_review`, `test_free_user_at_cap_cannot_review`, `test_redis_outage_fails_open`, `test_free_user_limited_to_3_interview_generations`, `test_usage_resets_monthly`, `test_monthly_window_default_unchanged`. Likely env-var-default related per slice 6.3's `FREE_DAILY_REVIEW_LIMIT=10` env-tunable note (slice B's LD-001 cap tightening 15→10 may have changed defaults that these tests pin). **Recommendation:** file a P3 hygiene B-row at next legitimate slice start to investigate + restore (run `git bisect` between `ba00331` and `309f6c4` to find the exact commit; verify env defaults vs test fixtures). NOT in CR's scope to fix; just surface. **Status: NEW — surfaced this regen via Step 0 BE baseline check.**

21. **NEW (process-improvement candidate) — D-026 cross-ref: spec authors must verify FE files referenced in mount-paths actually exist on disk, not just BE endpoints.** Slice 6.4 spec author at `309f6c4` referenced `pages/AdminAudit.tsx` in §8.5 + §11 6.4a AC-3 + §10.1 based on the existence of the BE endpoint `GET /api/v1/admin/audit` (E-018a, `3b43772`). Disk reality: only `pages/AdminAnalytics.tsx` + `pages/AdminPanel.tsx` exist; no `AdminAudit.tsx`. Slice 6.4a impl R19 fired at Step 1 audit, resolved via in-slice spec amendment per §12 D-14 (`cbf878f`) — `/admin/audit` dropped from sidebar, sidebar reduced 5 → 4 links; impl `b0806d0` honors the amended AC-3. Logged in SESSION-STATE Drift Flags as D-026 (RESOLVED). **Suggested SOP-5 enhancement (authored hint, not yet a rule):** when spec text says "Page X (Phase N) — untouched", `grep src/pages/` for `X.tsx` before signing off the spec. Filed as recommendation; Dhamo to lift to a numbered rule (R-class) if the pattern recurs. **Status: NEW — surfaced this regen via D-026 cross-ref.**
...
Walked `docs/specs/**/*.md` — **84 spec files across 7 phases** (+1 since prior regen at `d5f3d17`; net-new phase-6 spec #04 admin-authoring authored at `309f6c4` 2026-04-27 + amended in-slice at `cbf878f`). Status field = `^## Status` line OR `^**Status:**` bolded line at the top of the spec body — both styles are observed on disk; tooling that grepped only `^## Status` would undercount by 2 (specs #61, #62 use the `**Status:**` bolded form). Specs without either form are flagged "no status".
...
**Delta since prior regen at `d5f3d17`**: +1 phase-6 file (`04-admin-authoring.md` — `## Status:` heading-2 form). Phase-6 1/2/3 spec Status lines flipped to `Shipped (spec + impl) — closes B-0##` by `95bb3c5` (resolves §11 prior drift item 18). Phase-6 spec #04 in partial-shipped state (slice 6.4a shipped via `b0806d0`; slice 6.4b pending B-065) — Status line still reads `Drafted, not shipped` because no canonical post-ship form for partial-shipped specs exists yet; see §11 NEW item 19. Phase-5 unchanged at 36 / 14 / 22 (no new phase-5 specs since `d5f3d17`).
...
| 04-admin-authoring.md | Drafted, not shipped *(NEW — slice 6.4 spec author `309f6c4` 2026-04-27 + in-slice amendment `cbf878f` in slice 6.4a impl. **Slice 6.4a SHIPPED** — closes B-064 by `b0806d0` on 2026-04-27 (multi-route AdminLayout shell + AdminCards extraction + AdminDecks/AdminLessons placeholders + App.tsx route rewire; `pages/AdminPanel.tsx` deleted per §12 D-12). **Slice 6.4b PENDING** — B-065 🔴 still open (admin write CRUD + lesson_service body swap + fixture retirement + 4 editor pages + Markdown editor + confirm modal). Spec body Status line not flipped on impl-merge — partial-shipped state has no canonical post-ship form yet; see §11 NEW item 19.)* |
...
- **Total spec status hygiene gap:** 33 of 84 specs (39%) have no Status line. Concentration in phase-1 (7), phase-3 (3), phase-5 (22). The phase-6 sweep at `95bb3c5` flipped specs #01/#02/#03 to canonical post-ship form (closing prior CR §11 drift item 18 for those three specs), and also flipped phase-5 specs #53/#59/#60/#61/#62/#63. Phase-5 specs #51, #52, #57 remain unflipped despite shipping per BACKLOG — recommend follow-up sweep. Spec #04 (phase-6 admin-authoring) is partial-shipped (slice 6.4a shipped; slice 6.4b pending) and lacks a canonical Status string for that state — see §11 NEW item 19 for the recommended new form `Partially shipped — slice Na shipped (closes B-### by <sha>); slice Nb pending B-###`.
...
*End of snapshot. Generated 2026-04-27 at HEAD `f99a6b3` — targeted regen post-slice-6.4a (closes B-066). Sections in scope: §1 (header SHA + lineage + counts), §6 (FE route table — admin block rewrite + component graph 69→70), §7 (FE pages — `AdminPanel.tsx` row → 3 new `pages/admin/*` rows), §11 (drift item 18 ✅ RESOLVED + 3 NEW items #19/#20/#21), §13 (phase-6 row 3→4 + per-phase total 83→84 + new row for `04-admin-authoring.md`). Sections carried forward verbatim per LD-5: §2 (BE models — no model adds since slice 6.1's foundation 4-table batch), §4 (BE services — slice 6.4a is FE-only per AC-6), §5 (Alembic — no migration this slice), §8 (FE shared types — no new types this slice; slice 6.4b will add admin write-shape interfaces), §9 (known-dead — no new dead code), §10 (skills — `admin-panel.md` consulted but not edited), §12 (open questions — Q1–Q9 unchanged). **Lineage chain:** `17bf188` (§3 + §4 regen) → `7109542` (§1 + §2 + §5 regen) → `d5f3d17` (§6 + §7 + §8 + §9 + §10 + §11 + §12 + §13 regen, slice 6.3 final pass) → THIS commit (§1 + §6 + §7 + §11 + §13 targeted, post-slice-6.4a). **Next regen recommended after slice 6.4b (B-065) ships** — will add 13 new admin write routes (decks / lessons / quiz_items) + 3 new admin services + shared `admin_errors.py` + 4 FE editor pages + Markdown editor / confirm modal primitives + lesson_service body swap (fixture → DB query with `selectinload`) + `app/data/lesson_fixtures.py` deletion + 10 new admin PostHog events; that slice will touch §3 + §4 + §6 + §7 + §8 + §10 + §13.*
