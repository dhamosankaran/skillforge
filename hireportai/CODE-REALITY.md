...
High-signal output — re-verified against HEAD `<this-slice>` this targeted regen. Items 22 ✅ RESOLVED (admin write-shape interfaces enumerated in §8 this pass) + 23 NEW (informational pointer for B-072 pending impl); items 1-21 carry forward verbatim. **Reconciled against post-B-048 SOP state:** R3 = "Never skip auth" (auth only); R19 = push-back; R18 retired (merged into R15(c)); SOP-8 = concurrent-session detection (codified `2504d6b`); SOP-9 = no concurrent CC sessions on one tree (added by B-048, `e2714b4`); H1–H4 = chat-Claude ↔ CC handoff section (added by B-048).
...
22. ~~**NEW — §8 Frontend shared types lags `src/types/index.ts` reality.** Slice 6.4b-2 (`634f633`) shipped 8 new admin write-shape interfaces in `src/types/index.ts` plus 3 per-entity status-filter aliases. §8 of CR carries forward verbatim from prior regen per LD-5 (B-067 row scope explicitly excluded §8).~~ **✅ RESOLVED 2026-04-28 by `<this-slice>`** — this regen brought §8 in scope and enumerated all 11 admin write-shape entries (8 write-shapes + 3 per-entity status-filter aliases) in a new "Admin write-shape interfaces" sub-section, plus added a new "Backend analytics-event schemas" sub-section enumerating the 3 Pydantic v2 schemas shipped at slice 6.0 (`QuizReviewEventCreate` / `LessonViewEventCreate` / `LessonViewEventRequest`). §1 metadata Shared TS types row updated in lockstep.
