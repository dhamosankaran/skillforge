...
| Field | Value |
|-------|-------|
| Commit sha (short) | `246b4ec` (B-096 — targeted §1 + §2 + §5 + §11 + §13 regen + RC compaction; closes B-096). **Scope:** §1 fully refreshed; §2 / §5 / §11 / §13 patched for since-anchor delta. §3 / §4 / §6 / §7 / §8 / §10 carry detail rooted at the prior anchor `1ca046f` — chat-Claude should consult the slice-absorption log + the on-disk files for fresh detail until the next full-scope regen. **Prior anchor:** `1ca046f` (full regen 2026-04-30, B-086a + B-086b absorption). **Raw gap:** 37 commits. **Code-touching slice closes in window:** 8 — `95104d2` (B-084 slice 6.11 — admin content-quality dashboard + first non-NULL `lessons.quality_score` writeback), `2c92f11` + `2cf7c89` (B-088 + B-089 — E-043 §16 reconciliations IFD-1/4/5/6 + R-5 baseline-row backfill, cascade-closes drift D-029), `c77861f` (B-091 — `scripts/sha-backfill.sh` pilot), `d5602c3` (B-092 — process compaction bundle: SS+BACKLOG archives + `process-health-check.sh` pilot + R15(d) sharpening + LIGHT MODE §340 SOP-gate-narration rule), `d020f4d` (B-087 slice 6.13 — Pro daily digest opt-out + new `email_log` dedup table; alembic `f1a2b3c4d5e6`), `91be54f` (B-094a slice 6.13.5a — `card_quality_signals` foundation + critique-consumer + per-quiz_item user-aggregate writeback; alembic `c2b8a4d9e6f1` w/ NULLS NOT DISTINCT 5-tuple UNIQUE), `85860d5` (B-094b slice 6.13.5b — user-thumbs route + `<ThumbsControl />` FE + `LessonWithQuizzesResponse.viewer_thumbs` + admin thumbs columns; closes the 6.13.5 split at 20/20 ACs). Process commits in window: B-090 LIGHT MODE spec + impl, B-093 + B-095 (slice 6.13.5 spec-author + §12 amendment), 11 SHA-backfill commits, 1 audit (`3d03861` process-bloat scout), 3 drift logs (D-027 / D-030 / D-032). |
| Branch | `main` (last push to `origin/main` at `9834abe` per SESSION-STATE; 30+ commits queued unpushed; this regen + SHA-backfill queue on top of `246b4ec`). |
| Generated | 2026-05-02 (targeted regen at HEAD `246b4ec`). 37-commit raw gap with 8 code-touching slice closes; targeted scope per LIGHT-mode slice prompt. **Milestone callout:** Phase 6 quality-signal foundation now shipped — `card_quality_signals` table is the unified home for layer-1 critique + layer-2 per-quiz_item user-review + layer-3 user-thumbs signals; LD G2 cron decision (B-078 🟦) is now triggered for re-evaluation per `CODE-REALITY.md:848` precedent (chat-Claude / Dhamo own that decision). |
| Backend model files | **28** (`app/models/*.py`, excl. `__init__`, `request_models`, `response_models`). **31** ORM model classes (some files declare 2-3). **+2 since `1ca046f`**: `email_log.py` (B-087, `d020f4d`) + `card_quality_signal.py` (B-094a, `91be54f`). |
| Backend service files | **53** top-level (`app/services/*.py`, excl. `__init__.py`). **+5 since `1ca046f`**: `admin_content_quality_service.py` (B-084, `95104d2`), `email_log_service.py` (B-087, `d020f4d`), `card_quality_signal_service.py` (B-094a, `91be54f`), `critique_signal_consumer.py` (B-094a, `91be54f`), `thumbs_service.py` (B-094b, `85860d5`). Plus `services/llm/` 3 files + `app/jobs/` 1 file unchanged. |
| Backend schema files | **21** (`app/schemas/*.py`, excl. `__init__.py`). **+2 since `1ca046f`**: `admin_content_quality.py` (B-084, `95104d2`) + `card_quality_signal.py` (B-094a `91be54f` declared `CardQualitySignalWrite` + `CardQualitySignalRow`; B-094b `85860d5` appended `ThumbsRequest` + `ThumbsResponse`). `lesson.py` extended with `LessonWithQuizzesResponse.viewer_thumbs: Optional[ThumbsResponse]` per spec #12 §12 D-12 (B-094b). |
| Backend router files | **36** total = **30** v1 + 6 legacy. **+2 v1 routers since `1ca046f`**: `admin_content_quality.py` (B-084, `95104d2` — `GET /api/v1/admin/content-quality`), `thumbs.py` (B-094b, `85860d5` — `POST /api/v1/lessons/{id}/thumbs`). |
| Backend endpoints | **~92 unique decorators** (was 88) — added: B-084 admin content-quality dashboard read, B-087 PUT email_prefs Pro-tier guard branch, B-094b thumbs POST. `app/main.py` mount block now lists 30 v1 routers (was 28). |
| Alembic revisions | **31** (Head = `c2b8a4d9e6f1`). **+2 since `1ca046f`**: `f1a2b3c4d5e6_phase6_slice613_digest_opt_out_and_email_log.py` (B-087, down_revision `e043a1b2c3d4`) + `c2b8a4d9e6f1_phase6_slice6135a_card_quality_signals.py` (B-094a, down_revision `f1a2b3c4d5e6`; raw-DDL ALTER ADD on the 5-tuple UNIQUE NULLS NOT DISTINCT — see drift D-032). |
| Frontend pages | **28** — **+1 since `1ca046f`**: `pages/admin/AdminContentQuality.tsx` (B-084, `95104d2`). |
| Frontend components | **84** (excl. `__tests__/*`). **+4 since `1ca046f`**: `components/admin/content-quality/{DeckRollupTable,WorstLessonsTable,WorstQuizItemsTable}.tsx` (B-084) + `components/lesson/ThumbsControl.tsx` (B-094b). |
| Frontend utils | **7** utility .ts files in `src/utils/` (unchanged). `services/api.ts` extended with `fetchAdminContentQuality` (B-084) + `submitThumbs` (B-094b). |
| Shared TS types | `src/types/index.ts` (~793 lines, **+11 interfaces since `1ca046f`**): B-084 adds `DeckQualityRow` + `LessonQualityRow` + `QuizItemQualityRow` + `AdminContentQualityResponse` (extended in B-094a/b with `critique_scores` + `thumbs_aggregate` + `thumbs_count` + `pass_rate_persisted`); B-094b adds `ThumbsRequest` + `ThumbsResponse` + extends `LessonWithQuizzes.viewer_thumbs`. |
| Frontend hooks | **21** (`src/hooks/*.ts`, excl. `__tests__`). **+2 since `1ca046f`**: `useAdminContentQuality.ts` (B-084) + `useThumbs.ts` (B-094b — custom-hook idiom mirroring `useLesson`; optimistic state + revert on error). |
| Frontend context providers | 5 — unchanged. |
| Skills (tracked) | **22** in `.agent/skills/*.md`. Unchanged count. Content deltas: `analytics.md` (+`admin_content_quality_viewed` B-084, +`lesson_critique_signal_persisted` B-094a, +`lesson_thumbs_submitted` B-094b, `email_preferences_*` row corrections B-087, B-088 rescan reconciliations); `curriculum.md` §7 layer-2 critique-storage flipped future→active by B-094a; layer-3 split into 3 homes (lesson `lessons.quality_score` / per-quiz_item `card_quality_signals` user_review / user-thumbs `card_quality_signals` user_thumbs) by B-094a + B-094b. |
| Skills (untracked) | 3 directory-style — unchanged (see §10 + §12 Q8). |
| Prompts | `app/prompts/` 2 Markdown templates — unchanged. |
| Specs | **97 across 7 phases** (phase-0=6, phase-1=13, phase-2=8, phase-3=11, phase-4=6, phase-5=**37**, phase-6=**14**, **process=2**). **+4 since `1ca046f`** (phase-6 +2 / process +2): `12-quality-signals.md` (B-093 spec-author + B-095 §12 amendment) + `13-pro-digest-opt-out.md` (B-087 spec-author bundled with impl); NEW `docs/specs/process/` directory with `01-light-mode-reporting.md` (B-090) + de-facto follow-up `02-…` reserved. |
| Tests | **BE 802** / **FE 466** (B-094b final report). +89 BE / +38 FE since `1ca046f` baseline (713 / 428). Marker-gated integration: +2 alembic-roundtrip (B-087 `test_phase6_slice613_migration.py` + B-094a `test_phase6_card_quality_signals_migration.py`). **D-027** still tracking 5 env-var-cache-sensitive failures (carry-forward; see §11 #27). **D-028** alembic-roundtrip cascade: 10–25 fails on full-suite non-CI runs depending on slice; CI-canonical clean (`-m "not integration"`) per R13. R14 exception (b) — process slice, no test runs this regen. |

**Slice absorption (this regen):** B-084 (slice 6.11), B-087 (slice 6.13), B-088 + B-089 (E-043 reconciliations + D-029 cascade-close), B-091 (sha-backfill pilot), B-092 (process compaction bundle), B-094a (slice 6.13.5a foundation), B-094b (slice 6.13.5b UI). Plus B-090 process spec+impl, B-093 / B-095 (6.13.5 spec-author + §12 amendment), drift entries D-027 / D-030 / D-032.

**Milestone callout — Phase 6 quality-signal foundation.** With B-094a + B-094b shipped, `card_quality_signals` is now the unified storage shape per LD J2 for critique signals (write-time hook in `ingestion_worker.run_ingestion_job` Stage 2.5 post-`_persist_drafts` — see drift D-032 close-trail JC #1) + per-quiz_item user-review aggregate signals + per-user user-thumbs signals (5-tuple UNIQUE NULLS NOT DISTINCT — see drift D-032). Lesson detail GET seeds `<ThumbsControl />` initial state via additive `viewer_thumbs` field (spec #12 §12 D-12). The admin content-quality dashboard now surfaces lesson-level critique scores + per-quiz_item persisted pass_rate + lesson-level thumbs aggregate. **LD G2 cron decision (B-078 🟦) re-evaluation triggers** per `CODE-REALITY.md:848` precedent — chat-Claude / Dhamo own the cron architecture decision; not surfaced as a CR finding here.
...
**31 revisions** in `hirelens-backend/alembic/versions/`. Linear chain. **Head: `c2b8a4d9e6f1`** (was `e043a1b2c3d4` at `1ca046f`).
...
Walked `docs/specs/**/*.md` — **97 spec files across 8 directories** (phase-0..phase-6 + new `process/`). **+4 since `1ca046f`**: phase-6 +2 (`12-quality-signals.md` B-093 + §12 amendment B-095, `13-pro-digest-opt-out.md` B-087 spec-author bundled with impl), process +2 (`01-light-mode-reporting.md` B-090 — first spec under the new `docs/specs/process/` cross-phase workflow directory; slot 02 reserved for a future process spec).
