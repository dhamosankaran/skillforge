...
const CustomTooltip = ({ active, payload, label }: {
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-bg-overlay border border-white/10 rounded-lg p-3 text-sm shadow-lg">
      ...
      >
        ...
        <Bar dataKey="jd_count" name="In JD" radius={[3, 3, 0, 0]}>
          {chartData.map((entry, i) => (
            <Cell
              key={i}
              fill={entry.matched ? '#00ffc8' : '#f8514940'}
              stroke={entry.matched ? '#00ffc880' : '#f8514960'}
              strokeWidth={1}
            />
          ))}
        </Bar>
