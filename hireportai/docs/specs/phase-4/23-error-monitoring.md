# SPEC #23: Sentry Error Monitoring + Webhook Idempotency
...
### Backend
- `test_duplicate_webhook_is_idempotent` — send same event twice, verify
  plan changes only once, single `stripe_events` row.
- All existing payment tests pass unchanged.

### Frontend
- Trigger `throw new Error("test")` in a component — Sentry captures it.
- ErrorBoundary renders fallback with reload button.

### Manual Verification
- Set `SENTRY_DSN` → raise exception in a route → verify in Sentry dashboard.
- Set `VITE_SENTRY_DSN` → trigger frontend error → verify in Sentry dashboard.
