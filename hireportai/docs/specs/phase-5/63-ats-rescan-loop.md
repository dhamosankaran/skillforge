...
**§5.3 + §6.1 correction-amendment:** ✅ shipped at `1b86bf0` (2026-04-30) — spec §5.3 `scan_id` FK shape and §6.1 `score_resume_against_jd` signature corrected to match disk reality landed at `210dcb2` (B-086a). JC #2 + JC #3 from B-086a final report dispositioned in spec body. No further amendments expected before B-086b impl pickup.
...
**Post-impl reconciliations:** ✅ shipped at `13012be` (this slice — §16 NEW) — five drift items (IFD-1 + IFD-3 + IFD-4 + IFD-6 + W-1) surfaced by validation-probe `35350ea` reconciled in §16.1-§16.6. Slices 1+2 (B-088 + B-089 forward-filed 🔴) implement.
