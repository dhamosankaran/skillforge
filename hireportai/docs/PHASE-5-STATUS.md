...
| 5.17 | Interview Prep — store generated questions per JD | 🟡 PARTIAL | `docs/specs/phase-5/49-interview-question-storage.md` | Backend ✅ SHIPPED: `interview_question_sets` table (migration `f3350dcba3a5`), `app/models/interview_question_set.py`, `app/services/interview_storage_service.py::generate_or_get_interview_set`, `app/utils/text_hash.py::hash_jd`, route `app/api/routes/interview.py` serves cached on `(user_id, jd_hash)` hit and skips `check_and_increment` on cache hits. Tests in `tests/services/test_interview_storage.py` pin AC-1..AC-5b. | Slice commit pending (CODEX review). Frontend wiring (force_regenerate + cached-result UX) deferred as 5.17b — tracked in SESSION-STATE. **Note**: the `hash_jd` primitive did not previously exist on the scan-tracker path despite prior wording — this slice creates the shared util at `app/utils/text_hash.py` per the tracker Locked Decision; tracker can adopt it in a future slice. |
...
1. **5.17b — Interview-question storage frontend wiring.** Backend shipped in this slice (spec #49, migration `f3350dcba3a5`). Frontend still calls `/api/interview-prep` without `force_regenerate`; the "Regenerate Questions" button triggers cache-hits (no-ops against the user's intent). Next slice: add `force_regenerate` pass-through + "Generated from cache" indicator + "Regenerate (uses 1 of N)" copy on the re-run CTA.
2. **5.24 — Chat-with-AI during flashcard study (`CardChatPanel`).** No code surface yet. New LLM task (`card_chat`, reasoning tier), new side-panel UI on `/learn/card/:id`, Pro-gated. Scope is a single new route + component + LLM task. SESSION-STATE Frontend-test-coverage constraint already names this file as a future addition.
3. **5.25 — Admin LLM-driven analytics dashboard.** Spec exists as early draft (`01-admin-analytics-early-draft.md`). Promote to full spec, build on existing admin routes. Could be paired with 5.26.
4. **5.26 — Admin content-feed flow.** No spec. Define before implementation per CLAUDE.md Rule 14.
