...
| 5.17 | Interview Prep — store generated questions per JD | 🔴 PENDING | — | `app/api/routes/interview.py:42-43` calls `generate_interview_questions` **every request**. No storage model, no `jd_hash` column for interviews, no cache layer. `jd_hash` primitive exists only for the scan-tracker path. | Spec #35 refers to this pattern "for interview question storage" — the pattern is *specified* there for reuse, but the interview storage feature itself is not implemented. |
...
1. **5.17 — Interview-question storage per JD.** Today every call to `/api/interview-prep` regenerates via LLM; no cache. Pair the `hash_jd()` primitive (already used by scan-tracker auto-populate) with a new `interview_generations` table keyed on `(user_id, jd_hash)`; return cached on hit. Fastest-to-ship of the pending items and directly reduces LLM spend.
...
- **5.17 surprise**: v2.2-patch implied interview-question storage was spec'd-but-not-shipped; on disk there is *no* storage code **and** no dedicated spec file. The `hash_jd` primitive is reused in the scan-tracker path but not in the interview path. Net: further behind than the playbook suggested.
