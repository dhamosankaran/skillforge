...
| B-004 | results | Keyword Frequency Analysis colors don't match the legend | P1 | 🔴 | docs/specs/phase-5/21-analysis-results-improvements.md | P5-S21 (part) | Root cause CAUSE-2 per spec #21: hardcoded violet `rgba(124,58,237,0.5)` in `KeywordChart.tsx:83` vs themed `bg-accent-secondary/50` legend — violet is not `--accent-secondary` in any theme. Fix + impl in P5-S21b. |
...
| E-010 | results | Add education layer (info icons + first-visit guided tour) | P1 | 🔴 | docs/specs/phase-5/21-analysis-results-improvements.md | P5-S21 (part) | Pairs with B-004. Info-icon tooltips layer specced in #21 (P5-S21b closes that half); guided tour deferred per spec §Out of Scope. |
