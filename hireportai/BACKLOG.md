...
| E-037 | security | Auth + rate limit on `/api/v1/rewrite` and `/api/v1/rewrite/section` | P2 | 🔴 | | (B-001 post-mortem 2026-04-21) | Both `/api/v1/rewrite` and `/api/v1/rewrite/section` are unauthed and call Pro-tier Gemini (`resume_rewrite` + `resume_rewrite_section` tasks in `REASONING_TASKS`). Anyone hitting these endpoints spends tokens on our account. `/api/v1/rewrite/section` is new as of B-001 (commit `167b70f`, spec #51); `/api/v1/rewrite` is pre-existing (re-exports legacy unauthed `/api/rewrite`). Parent parity was the reason the new endpoint shipped unauth, but a new endpoint inherits the posture deliberately, not accidentally — this deserves tracking. **Scope when picked up:** (a) audit current rate-limiting config on both endpoints (if any); (b) add auth (`get_current_user` or `get_current_user_optional`) + per-user rate limit; (c) decide whether legacy `/api/rewrite` gets the same treatment or stays parity-unauth pending Phase-6 legacy cleanup. **Acceptance:** `/api/v1/rewrite` and `/api/v1/rewrite/section` require auth OR have a tight per-IP rate limit; unauth requests return structured 401/429 with no Gemini call; token-cost exposure from unauth traffic drops to zero (measurable via PostHog). **Cross-ref:** B-001 post-mortem judgment call #6; Phase-6 legacy-route cleanup (tracked separately); spec #51 §2 (out of scope for B-001 impl). **Distinction from E-036:** E-036 is Stripe-surface integration testing (paywall/checkout/webhook); E-037 is Lens-surface (rewrite) auth/cost-exposure. Different concerns despite both being "endpoint hardening." |
