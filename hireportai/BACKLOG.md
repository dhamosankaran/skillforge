...
| E-014 | billing | Paywall dismissal flow + win-back email | P1 | 🟡 | docs/specs/phase-5/42-paywall-dismissal.md | P5-S26b | (spec ✅, impl 🔴) Dismissal logged; re-prompt only after N actions; win-back email at 3rd dismissal. Spec authored on 2026-04-20; impl slice (P5-S26b-impl) pending. Impl prereqs: new `user.downgraded_at` column (LD-5), `STRIPE_WINBACK_COUPON_ID` env, new `paywall_winback.html` template. |
