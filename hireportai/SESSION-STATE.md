...
| Field | Value |
|-------|-------|
| **HEAD commit** | `e74d1f2` (P5-S20; prev P5-S21b commit `1c0817a`) |
| **Branch** | `main` |
...
**P5-S22** — Plan-aware "missing skills → flashcards" CTA on `/prep/results` (BACKLOG E-011). Pro users get a deep link to the flashcard set for the missing skill; free users get the upgrade modal. Reuses the ATS-card-bridge mapping. Spec backfill may be needed — check if `docs/specs/phase-5/` has a slot for E-011 before starting (no P5-S22 spec file exists yet per BACKLOG).
