...
| Field | Value |
|-------|-------|
| **HEAD commit** | `eae7ff7` (P5-S26b spec #42 — paywall dismissal + win-back, spec-authored, impl pending) |
| **Branch** | `main` |
| **CODE-REALITY.md sha (repo)** | `0a161d1` (regenerated 2026-04-20) — **STALE** (P5-S22b left `MissingSkillsPanel.tsx` / `Results.tsx` diverged; this spec-authoring slice did not touch code, so staleness is unchanged). |
| **CODE-REALITY.md in chat Project** | Stale ❌ — re-upload after next regen |
| **CODE-REALITY stale (either copy)?** | Yes — regenerate before next plan-level prompt (unchanged by this slice; docs-only) |
| **Last hand-edit** | 2026-04-20 — P5-S26b spec #42 authored (paywall dismissal + win-back); impl slice pending |
...
**P5-S26b-impl** — Implement spec #42 (paywall dismissal + win-back). Spec authored 2026-04-20 at `docs/specs/phase-5/42-paywall-dismissal.md` (commit `eae7ff7`); 8 LDs locked in §3; implementation slice has three explicit prereqs (see spec §9 + §12):
  (a) Alembic migration — new `paywall_dismissals` table + `user.downgraded_at` TIMESTAMPTZ column,
  (b) wire `user.downgraded_at` setter in `app/services/payment_service.py::_handle_subscription_deleted`,
  (c) `STRIPE_WINBACK_COUPON_ID` env var wired through `app/core/config.py` + `app/templates/paywall_winback.html` drafted.
Route prefix deviation from v2.2 stub: spec uses `/api/v1/payments/*` (matches live convention), NOT `/api/v1/billing/*`. BACKLOG E-014 flipped 🔴 → 🟡 (spec ✅, impl 🔴).
