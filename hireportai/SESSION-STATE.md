...
- **5.17b — Interview-question storage frontend wiring** (opened by spec #49, 2026-04-19). Backend cache shipped; the frontend still calls `POST /api/interview-prep` without `force_regenerate`, and the "Regenerate Questions" button silently returns cache-hits. **Definition of done:** `useInterview.runInterviewPrep` accepts an optional `{ forceRegenerate }`, `services/api.ts::generateInterviewPrep` forwards it, `Interview.tsx` exposes "Regenerate (uses 1 of your N)" copy and surfaces the new `cached` + `generated_at` response fields (e.g., a "Loaded from cache" chip with a timestamp). Test: at least one Vitest that asserts the hook passes `force_regenerate: true` when the regenerate CTA is clicked.
...
- **Backend**: 248 unit passed, 6 integration deselected (last run: spec #49)
- **Frontend**: 111/111 passing (last run: spec #49 — unchanged; no FE work in this slice)
