...
| Field | Value |
|-------|-------|
| **HEAD commit** | `2312cc9` (CODE-REALITY regen; prev `d155dcb` P5-S22-WALL-b) |
| **Branch** | `main` |
| **CODE-REALITY.md sha (repo)** | `2312cc9` (regenerated 2026-04-20) — **CURRENT** |
| **CODE-REALITY.md in chat Project** | Stale ❌ — re-upload before next planning prompt |
| **CODE-REALITY stale (either copy)?** | Chat-Project copy only — re-upload |
| **Last hand-edit** | 2026-04-20 — CODE-REALITY.md regenerated to match HEAD |
...
**P5-S22b** — Plan-aware "missing skills → flashcards" CTA on `/prep/results` (BACKLOG E-011). Pro users get a deep link to the flashcard set for the missing skill; free users get the "Study these cards — free preview" CTA routing into `/learn?category=<id>` (revised per LD-001 — the free preview now has a real wall to hit thanks to P5-S22-WALL-b). Spec at `docs/specs/phase-5/22-plan-aware-missing-skills-cta.md`.

P5-S22-WALL (spec #50) shipped in P5-S22-WALL-b this slice, unblocking both P5-S22b (free-preview semantics) and P5-S26b (paywall dismissal — now has a real paywall to dismiss).
...
1. 2026-04-20 — CODE-REALITY.md regenerated to match HEAD `2312cc9`. Targeted updates: Section 1 metadata (sha / date / FE components 59→60 / specs 66→69), Section 3 (added pre-existing `GET /api/v1/admin/ping` row + 402-branch annotation on `POST /api/v1/study/review`), Section 4 (`study_service.py` Public names += `DailyReviewLimitError`; one-line note on the `_check_daily_wall` Redis helper), Section 6 (new "Wall-aware components" trailing paragraph pointing at `QuizPanel.tsx`), Section 7 (Results.tsx events column += `job_fit_explanation_viewed`, `results_tooltip_opened`). Models/Alembic/Services table structure unchanged. `app/utils/timezone.py` is out of structural scope (no `app/utils/` section in CODE-REALITY); noted in audit only. No code changes, no BACKLOG close, no test changes.
2. P5-S22-WALL-b — Free-tier daily-card review wall (spec #50, closes E-030). `_check_daily_wall` in `study_service.py` + `app/utils/timezone.py::get_user_timezone`; 402 branch on `POST /api/v1/study/review`; `QuizPanel.tsx` wires the modal. Tests BE 254 → 265, FE 142 → 147. Commit `d155dcb`.
3. P5-S20 — Moved Job Fit Explanation above the fold on `/prep/results`. Flattened the 3-panel grid to 11 direct grid children so DOM order = mobile order = tab order; was: left-sidebar → main → right panel put Job Fit at DOM pos 5+ on mobile (behind ATS + Score Breakdown + Nav + 3 CTA buttons). New order: ATS → Job Fit → Missing Skills → Keywords → Score Breakdown → Skills Radar → Bullets → Formatting → Improvements → Nav → CTAs. Desktop layout preserved via explicit `lg:col-start-* lg:row-start-*` + `xl:col-start-*` overrides + `lg:col-span-2` fallback on LG for right-panel items. `lg:sticky lg:top-20` now on the ATS grid item; `xl:sticky xl:top-20` now on Missing Skills. All 9 section IDs preserved (`results_tooltip_opened` enum stable); all 9 tooltip copies preserved verbatim. New PostHog event `job_fit_explanation_viewed {view_position: 'above_fold'}` fires once per mount via useRef idempotency guard (matches `home_dashboard_viewed` convention); catalogued in `.agent/skills/analytics.md`. Tests 134 → 142 (+8): `tests/pages/Results.ordering.test.tsx` via `compareDocumentPosition`. tsc clean. BE unchanged. BACKLOG E-009 🔴 → ✅. No CODE-REALITY regen script exists on disk; stale at sha `f09be80` from previous slice — regenerate before next plan-level prompt.
4. P5-S21b — Keyword color fix + education tooltips. `KEYWORD_LEGEND` + `rgbaFromCssVar` helper replace hardcoded violet with theme-aware rgba across 3 themes. `PanelSection` primitive extracted; hardened with Escape-dismiss + click-outside + `role="tooltip"` + `aria-describedby` + `aria-expanded`. Tooltips on 5 previously-uncovered sections + Score Breakdown refactor. New `results_tooltip_opened` event (9-value section enum). Tests 114 → 134 (+20). BACKLOG B-004 + E-010 info-icon half closed.
5. P5-S18 — HomeDashboard + widget catalog. `DashboardWidget` primitive (`loading/data/empty/error` states). Seven widgets under `src/components/home/widgets/`: TodaysReview, Streak, WeeklyProgress, LastScan, InterviewTarget (display-only), Countdown (Mode 1 inline date-setter → `updatePersona({persona, interview_target_date})`; Mode 2 wraps `mission/Countdown` + active-mission CTA), TeamComingSoon (action-less — no waitlist component on disk). Persona-keyed render modes (`home-mode-<persona>`), 1/2/3 responsive grid, greeting with first-name fallback, `home_dashboard_viewed` with useRef guard. `App.tsx` swapped `HomeDashboardPlaceholder` → `HomeDashboard`; placeholder deleted; `tests/App.redirects.test.tsx` updated. FE tests 38 → 78 (+40). BE 184/184 unchanged. AC-9 persona-in-PATCH-body regression assertion in `CountdownWidget.test.tsx`.
