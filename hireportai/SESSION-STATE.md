...
| Field | Value |
|-------|-------|
| **HEAD commit** | `1c0817a` (P5-S21b; prev spec-author commit `fe4a333`) |
| **Branch** | `main` |
...
**P5-S20** — Move Job Fit Explanation above the fold on `/prep/results` (BACKLOG E-009). P5E chunk remaining items: P5-S20 and P5-S22 (B-004 + E-010 were both closed in P5-S21b this commit). P5-S20 coordination note from spec #21 §Dependencies: P5-S20 must preserve the section IDs (`job-fit`, `keywords`, `skills`, `bullets`, `ats-score`, `score-breakdown`, `missing-skills`, `formatting`, `improvements`) referenced by the `results_tooltip_opened` analytics enum. If structural reshuffling changes DOM anchor IDs, update the analytics enum in the same commit.

After P5E, continue per execution order in `claude-code-prompts-all-phases-v2.md` reconciled against `docs/PHASE-5-STATUS.md`:

1. P5E — P5-S20, P5-S22 (P5-S21 closed in this commit)
...
**Bold = added in v2.2 patch.** P5-S9/S10/S11/S12/S13/S14/S16/S17/S18/S18b/S18c/S21 all shipped.
...
- **Frontend test coverage**: Every new page added in Phase 5 must ship with at least one Vitest test. Current frontend count: **134/134** (last run: P5-S21b). Grow with every UI slice.
...
- **Frontend (Vitest)**: 134/134 passing (last run: P5-S21b — 114 → 134 with `tests/components/KeywordChart.colors.test.tsx` + `tests/components/PanelSection.tooltip.test.tsx` + `tests/pages/Results.tooltips.test.tsx`)
