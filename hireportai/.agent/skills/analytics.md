...
| `rescan_failed` | `app/api/routes/analyze.py` (`rescan_application`) | `{tracker_application_id, error_class: 'scoring_error'}` — fires on the 502 path before the HTTP error is raised. Counter is NOT incremented (request rolls back). Spec #63 §9 + AC-8. |
