...
| `home_status_hero_rendered` | `components/home/HomeStatusHero.tsx` (mounted by `pages/HomeDashboard.tsx`) | `{persona: 'interview_prepper' \| 'career_climber', plan: 'free' \| 'pro' \| 'enterprise', clauses_shown: string[]}` — fires once per mount via `useRef` idempotency guard, only after at least one clause resolves (so a `null`-render mount does not fire). `clauses_shown` enumerates which template slots contributed visible copy this mount (IP enum: `'company' \| 'days' \| 'due' \| 'due_zero' \| 'score'`; CC enum: `'streak' \| 'streak_zero' \| 'due' \| 'due_zero'`). Collapses the originally-planned `home_status_hero_data_missing` second event into the same fire — same telemetry signal, fewer events (spec #65 §9.1 / D-6; B-120; closes E-050). |
| `loop_frame_rendered` | `components/dashboard/LoopFrame.tsx` (mounted by `pages/Results.tsx`) | `{surface: 'results', current_step: 1 \| 2 \| 3 \| 4, has_interview_date: boolean, plan: 'anonymous' \| 'free' \| 'pro'}` — fires once per mount via `useRef` idempotency guard (matches `home_dashboard_viewed` / `paywall_hit` / `home_study_gaps_prompt_shown` convention). `surface` widened to `'results' \| 'appshell'` by spec #66 / B-122; the AppShell mount **suppresses** this event and fires `loop_strip_rendered` instead per §9.2 / D-4 (no double-counting). The four-step copy ("Scanned" / "Studying" / "Re-scan" / "Interview") is locked verbatim per spec #64 §14 OQ-2 so E-051 inherits without drift (spec #64 §9.1 / B-119; closes E-049 part (b)). |
| `loop_strip_rendered` | `components/layout/LoopProgressStrip.tsx` | `{persona: 'interview_prepper', plan, current_step: 1 \| 2 \| 3 \| 4, has_overdue: boolean, days_until_interview: number \| null, tracker_id: string}` — fires once per mount via `useRef` keyed on `tracker_id` per D-7 after at least step 4 resolves. Mount-level idempotency: navigation away/back IS a fresh mount and DOES refire (spec #66 §9.1; B-122; closes E-051). |
| `loop_strip_step_clicked` | `components/layout/LoopProgressStrip.tsx` | `{step: 3, current_step, plan}` — fires before the rescan deep-link navigation. Step 3 is the only clickable step (per spec §8.4); step 1/2/4 clicks are not bound (spec #66 §9.1 / B-122). |
| `loop_strip_rescan_unlocked` | `components/layout/LoopProgressStrip.tsx` | `{plan, days_since_last_scan, percent_reviewed}` — fires once when step 3 transitions `'locked'` → `'current'`. Per-mount `useRef<boolean>` tracks prior-locked state (spec #66 §9.1 / D-12; B-122). |
| `loop_strip_step_completed` | `components/layout/LoopProgressStrip.tsx` | `{step: 1 \| 2 \| 3, plan, days_in_step: number \| null}` — fires once per step per mount when a step transitions to `'done'`. Per-step `useRef<Set<LoopFrameStep>>` tracks fired-set per D-12. `days_in_step` is null on first observation; future enhancement could thread per-step timestamps (spec #66 §9.1 + §13 forward-work; B-122). |
...
| `lesson_thumbs_submitted` | `components/lesson/ThumbsControl.tsx` | `{lesson_id, score: -1\|1, previous_score: -1\|0\|1, persona, plan}` — fires every time the user clicks 👍 / 👎 on a lesson via the `useThumbs.submit` mutation (Phase 6 slice 6.13.5b / B-094b / spec #12 §9.1). `previous_score` captures the user's prior thumbs (0 = first submission) so the funnel can split first-vote vs vote-flip behaviour. Sticky thumbs v1 (§12 D-11) — POST with score=0 is rejected at the schema boundary; this event never fires with `score: 0`. Lesson-level only v1 per §12 D-7; per-quiz_item event was DROPPED before impl. |
...
| `lesson_critique_signal_persisted` | `app/jobs/ingestion_worker.py` (Stage 2.5 hook → `critique_signal_consumer.persist_critique_signals`) | `{admin_id, job_id, lesson_count: int, dimension_count: int, rows_written: int, internal: true}` — info-only event; fires once per ingestion job after the four per-dimension critique scores land in `card_quality_signals`. `rows_written` = `lesson_count × dimension_count` on first persist; on idempotent re-runs the value is the same (UPSERT touches every key) but no new rows exist on disk. Skipped entirely when `rows_written=0` (no lessons to attach signals to OR critique payload had no dimensions). Phase 6 slice 6.13.5a / B-094a / spec #12 §9.1 + §12 D-3. `internal: true` keeps the event out of user-facing PostHog dashboards. |
| `pro_digest_sent` | `app/services/pro_digest_service.send_pro_digest` (post-`record_send`) | `{user_id, plan: 'pro'\|'enterprise', cards_due: int, streak: int, has_mission: bool, has_recent_scan: bool, has_aggregate_block: bool, resend_id: str\|null, internal: true}` — fires once per Pro user per cron tick after `email_log` row commits. `resend_id` is the Resend message id; `null` when `RESEND_API_KEY` is unset (dev/CI no-op). `has_aggregate_block` (added by spec #67 §9.2 / B-125a) is `true` when the digest contained the Career-Climber aggregate intent block. Phase 6 slice 6.14 / B-098 / spec #14 §9.1 + §12 D-10. `internal: true` keeps the event out of user-facing PostHog dashboards. |
| `career_intent_captured` | `app/api/v1/routes/career_intent.py` (`POST /api/v1/users/me/career-intent`) | `{target_role, target_quarter, source}` — fires on first-time intent capture (no prior current row) for Career-Climber users. `source` derives from the `X-Capture-Source` request header (default `'api'`); typical values `'persona_picker'` / `'profile_edit'`. Spec #67 §9.1 / B-125a / E-052. |
| `career_intent_updated` | `app/api/v1/routes/career_intent.py` (POST when prior current row exists; DELETE clear path) | `{from_role, to_role, from_quarter, to_quarter, source}` — fires when an intent is changed (POST supersedes prior current row) or cleared (DELETE; `to_role`/`to_quarter` are `null`). `source` derives from `X-Capture-Source` header; DELETE defaults to `'profile_clear'`. Spec #67 §9.1 / B-125a / E-052. |
| `career_intent_email_block_rendered` | `app/services/pro_digest_service.send_pro_digest` (post-`record_send`, gated on `payload.aggregate_intent_block is not None`) | `{target_role, target_quarter, cohort_size, top_category, internal: true}` — fires once per digest send that contained the Career-Climber aggregate block. `top_category` is the highest-share category name (e.g. `"system design"`) — used for downstream A/B analysis on which category headlines drive digest engagement. Per §12 D-13, fires ONLY on actual send-success path (template-render-only paths do NOT fire). Spec #67 §9.1 / B-125a / E-052. |
| `pro_digest_intent_aggregate_failed` | `app/services/pro_digest_service.compose_digest` (`_aggregate_intent_block` catch path) | `{user_id, error_class, internal: true}` — fires when the career-intent read or aggregate-stats query raises (DB connection, query timeout, etc.). The composer treats the failure as `aggregate_intent_block=None` and the user still receives the digest with the existing three sections (graceful degradation per §6.5). Spec #67 §9.3 / B-125a. |
| `pro_digest_skipped_dedup` | `app/services/pro_digest_service.send_pro_digest` (pre-compose `was_sent_today` short-circuit OR post-send `record_send` IntegrityError concurrent-tick race) | `{user_id, internal: true}` — fires when the `email_log` row for `(user_id, 'pro_digest', today_utc)` already exists. Idempotency contract: re-running the cron in the same UTC day produces all `skipped_dedup` for already-sent users. Phase 6 slice 6.14 / B-098 / spec #14 §9.1 + §12 D-10. |
| `pro_digest_skipped_empty` | `app/services/pro_digest_service.send_pro_digest` (compose returns None) | `{user_id, plan: 'pro'\|'enterprise', internal: true}` — fires when `compose_digest` returns None per §12 D-7 strict empty-rule (cards_due == 0 AND !mission_active AND last_scan_score is None). Zero noise to dormant Pro users. Phase 6 slice 6.14 / B-098 / spec #14 §9.1. |
| `pro_digest_failed` | `app/services/pro_digest_service.send_pro_digest` (`EmailSendError` OR compose raises OR `record_send` SQLAlchemyError) | `{user_id, plan: 'pro'\|'enterprise', error_class: 'send_error'\|'compose_error'\|'record_error', internal: true}` — fires WITHOUT a `record_send` write per §12 D-8 (next-tick retry). `send_error` = Resend permanent failure after 3× retry exhaustion (3-retry handled by Phase-2 `email_service`); `compose_error` = `compose_digest` raised; `record_error` = DB error during `record_send` (rare; `IntegrityError` is treated as `skipped_dedup` separately, not `failed`). Phase 6 slice 6.14 / B-098 / spec #14 §9.1 + §12 D-8 / D-10. |
